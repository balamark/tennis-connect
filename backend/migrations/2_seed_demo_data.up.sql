...
-- Check if we're in development environment before seeding
DO $$
BEGIN
    -- Only seed demo data if APP_ENV is development or not set
    IF COALESCE(current_setting('app.environment', true), 'development') = 'development' THEN
        ...
        RAISE NOTICE 'Demo data seeded for development environment';
    ELSE
        RAISE NOTICE 'Skipping demo data seeding - not in development environment';
    END IF;
END $$; 